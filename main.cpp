...
public:
    ...
    static void publishOpenCommChannelResponse(struct mosquitto *mosq, const struct mosquitto_message *message) {
...
private:
    ...
    static void messageCallback(struct mosquitto* mosq, void* userdata, const struct mosquitto_message* message) {
        if (message->topic == std::string(TOPIC_READ_PGN_REQUEST)) {
            ...
            publishOpenCommChannelResponse(mosq, message);
       }
    }
};
